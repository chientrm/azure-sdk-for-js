...
function DeployStressPackage(
    ...
    $dockerFilePath = if ($pkg.Dockerfile) {
        [System.Io.Path]::Combine($pkg.Directory, $pkg.Dockerfile).ToString()
    } else {
    ...
    if ($pushImages -and (Test-Path $dockerFilePath)) {
        ...
        $dockerBuildFolder = if ($pkg.DockerBuildDir) {
            ...
            $dockerFile.DirectoryName.Trim()
        }
        $dockerBuildFolder = [System.IO.Path]::GetFullPath($dockerBuildFolder)
